...
public class DataNodeSyncUtil {
    public static final List<String> deviceTypeList = new ArrayList<>(Arrays.asList("Gnss", ""));
    ...
    public static <T extends StaticInfoData> void SyncNoLinkDataNodeWhenDataChanged(
        ...
        for(T aData: dataList){
            String linkCodeOfDevice = aData.getUid();
            ...
            if(linkedNodeList.isEmpty()) {
                ...
                DataNode newNode = dataNodeRepo.save(
                        DataNode.dataNodeBuilder().name(aData.getName()).bank(parentNode.getBank())
    ...
    public static void SyncDeviceNodeWhenDataChanged(
            List<MonitorInfo> monitorInfoList, IDataNodeRepo dataNodeRepo, DataNode parentNode) {
        List<String> linkCodeList = new ArrayList<>();
//        DataNode parentNode = dataNodeRepo.getNodeByCategoryAndName("DeviceInfoGroup", "deviceInfo");
        ...
        for(MonitorInfo aData: monitorInfoList) {
            ...
            // add node process
            if(linkedNodeList.isEmpty()) {
                String deviceType = deviceTypeList.get((aData.getType()-1));
