...
export default class FlowFieldLayer{
        ...
        if(type === 'Float'){
            ...
            .then((response)=>{
                ...
                .then((bitmap)=>{
                    ...
                    //readPixels() 方法从当前的颜色帧缓冲（framebuffer）中读取指定矩形的像素矩阵
    ...
    async prepare(gl:WebGL2RenderingContext) {
        ...
        this.updateShaderObj = await this.init2ShadersFromSrc(
            gl,
            '/scratchSomething/flowWebGL/shaders/update.vert',
            '/scratchSomething/flowWebGL/shaders/update.frag',
            XF_Varings
        )
        this.trajectoryShaderObj = await this.init2ShadersFromSrc(
            gl,
            '/scratchSomething/flowWebGL/shaders/trajectory.noCulling.vert',
            '/scratchSomething/flowWebGL/shaders/trajectory.noCulling.frag',
        )
