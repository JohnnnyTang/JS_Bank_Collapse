...
export class bankRiskWarn {
    ...
    static getProfileData = (before, now, profileId) => {
        ...
        const url = profileId < 10 ?
            `${import.meta.env.VITE_BASE}/bankRiskWarnStaticJson/fixedSection-JC0${profileId}_beg${before}_end${now}.json` :
            `${import.meta.env.VITE_BASE}/bankRiskWarnStaticJson/fixedSection-JC${profileId}_beg${before}_end${now}.json`;
        return axios.get(url)
    }
