    ...
    <div v-show="dataListShow">
      ...
      <div class="add-data-dialog" v-else>
        ...
        <div class="add-data-card">
          <div class="head">
            <div>
              筛选类别：<strong>{{ selectData === "" ? "默认所有" : selectData }}</strong>
            </div>
            <div class="right">
              已选择
...
const { fuzzyQueryDataList, findFiles } = ModelRequest;
import utils from "@/utils/CommonUtils";
const { imgBase64 } = utils;
import { classTree } from "../dataConfig.js";
export default defineComponent({
  ...
  setup(_, context) {
    ...
    const dataList = ref([]);
    const selectDataList = ref({ id: "", name: "", avatar: "" });
    const tableData = ref([]);
    const fileList = ref([]);
