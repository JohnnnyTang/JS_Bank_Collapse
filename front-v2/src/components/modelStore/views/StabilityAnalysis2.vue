  ...
  <div class="all">
    ...
    <div class="model-content-container">
              ...
              >
                <template #default="{ node, data }">
                  <div class="evolution-tree-node" style="width: 100%">
                    <el-dropdown
                      @command="handleEvolutionTreeCommand"
                      trigger="contextmenu"
                      placement="bottom-end"
                      style="width: 100%"
                    >
                      ...
                      <template
                        #dropdown
                        v-if="data.type === 'dem' || data.type === 'section-geojson'"
                      >
                        <el-dropdown-menu>
            ...
            <div class="dp-content" style="height: 32vh">
              ...
              >
                <template #default="{ node, data }">
                  <div class="evolution-tree-node" style="width: 100%">
                    <el-dropdown
                      @command="handleEvolutionLayerCommand"
                      trigger="contextmenu"
                      placement="bottom-end"
                      style="width: 100%"
                    >
      ...
      <div class="main">
        ...
        <div class="model-container" v-if="isModelPanelShow">
          <div style="font-size: large; padding-bottom: 16px">近岸演变分析模型</div>
          <div class="model-list">
            <el-card style="height: 300px; object-fit: cover">
              ...
              <div style="padding: 4px 0px">
                <el-button @click="handleSelectModelClick('section-graph')"
                  >断面形态</el-button
                >
              </div>
            </el-card>
        ...
        <div class="model-params-container" v-if="modelType">
          <div style="font-size: large; padding-bottom: 16px">模型参数设置</div>
          <el-form
            ref="paramsRef"
            :rules="rules"
            :model="paramsInfo"
          ...
          >
            ...
            <el-form-item>
              <el-button type="primary" @click="runModel(paramsRef, modelType)"
                >运行模型</el-button
              >
    ...
    <div class="analysisCenter" v-show="showAnalysisCenter">
      ...
      <div
        class="back"
        @click="
          showAnalysisCenter = !showAnalysisCenter;
          checky1 = true;
          checky2 = false;
        "
      ></div>
...
import { demoJson, drawRateGraph, drawSectionGraph } from "../stability-sub/util.js";

const tileServer = import.meta.env.VITE_MAP_TILE_SERVER;
const showHyCondition = ref(false);
const showStParams = ref(false);
const showModelStatus = ref(false);
const buttons = ref(["水文条件选择", "模型参数设置", "模型计算", "模型运行状态"]);
...
const radio2Click = () => {
  ...
  if (map) {
      ...
      !map.getLayer("ras-" + dem) &&
        map.addLayer({
          ...
          paint: {
            ...
            "raster-color-mix": [DEMMINMAXMAP[dem].max - DEMMINMAXMAP[dem].min, 0, 0, 0],
            "raster-opacity": 1,
            "raster-color-range": [0, DEMMINMAXMAP[dem].max - DEMMINMAXMAP[dem].min],
          },
          layout: {
            visibility: "none",
          },
    ...
    map.on("draw.create", function (e) {
      ...
      let startWebMerCoord = convertToMercator(lineFeature.geometry.coordinates[0]);
      let endWebMerCoord = convertToMercator(lineFeature.geometry.coordinates[1]);
...
const handleEvolutionLayerCommand = (command) => {
  console.log(command);
  if (selectedEvolutionNode.value && evolutionLayerData.value && command === "delete") {
      ...
      if (!map.getLayer("ras-" + data.label)) {
        map.addLayer({
          ...
          paint: {
            ...
            "raster-color-mix": [
              DEMMINMAXMAP[data.label].max - DEMMINMAXMAP[data.label].min,
              0,
              0,
              0,
            ],
        ...
        map.setLayoutProperty("ras-" + data.label, "visibility", "visible");
      }
    }
  } else {
    if (data.type === "section-geojson") {
