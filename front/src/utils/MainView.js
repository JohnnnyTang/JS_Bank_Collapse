...
const initAllLayer = async (map) => {
    ...
    if (map.loaded()) {
        ...
        console.log('map load 在 请求之前', map.loaded());
        //map add source
        ...
        console.log('add source and layer ok');
    }

    //map load 在 请求之后
    else {
        console.log('map load 在 请求之后', map.loaded());
        map.on('load', () => {
            map.addSource('channelSource', {
                'type': 'geojson',
                'data': channel_gs
